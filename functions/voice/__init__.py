...
async def voice(
    ...
    if message_raw[:2] == "草草":
        ...
        if not sp_m[1] in n.keys():
            await app.sendGroupMessage(group, MessageChain.create([At(member.id),Plain(" 发音人选择错误！")]))
            return

        elif len(message.asDisplay()) > 50:
            await app.sendGroupMessage(group, MessageChain.create([At(member.id), Plain(" 字数过多！")]))
            return

        if len(sp_m) < 3:
            await app.sendGroupMessage(group, MessageChain.create([At(member.id),Plain(" 请按照格式发送！格式如下：\n草草 发音人 文本")]))
            return
